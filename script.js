...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateMoneyZakat() {
        ...
        if (goldPricePerGram <= 0) {
            ...
            showGoldPriceModal("لحساب الزكاة", function(price) {
                if (price > 0) {
        ...
        const nisabReached = moneyAmount >= goldNisabValue;
...
function showSilverPriceModal(purposeText, callback) {
    ...
    const modalContainer = document.createElement('div');
    modalContainer.className = 'price-modal silver-modal';
    ...
    modalContainer.innerHTML = `
        <div class="modal-header">
            ...
            <h3>نحتاج إلى معرفة سعر الفضة</h3>
        </div>
        <div class="modal-body">
            <p>تذكير: <span class="highlight">${purposeText}</span>، نحتاج إلى معرفة سعر جرام الفضة بالريال</p>
            <div class="quran-reminder">
                ﴿ وَالَّذِينَ يَكْنِزُونَ الذَّهَبَ وَالْفِضَّةَ وَلَا يُنفِقُونَهَا فِي سَبِيلِ اللَّهِ فَبَشِّرْهُم بِعَذَابٍ أَلِيمٍ ﴾
                <span class="sura-info">سورة التوبة: آية ٣٤</span>
            </div>
            <div class="input-silver-container">
                <label for="modal-silver-price">سعر جرام الفضة (بالريال):</label>
                <input type="number" id="modal-silver-price" placeholder="أدخل السعر هنا" min="1" step="0.01">
            </div>
        </div>
        <div class="modal-footer">
            <button class="modal-confirm-btn pulse-animation">تأكيد وحساب الزكاة</button>
    ...
    function updateNisabValues() {
        if (goldPricePerGram > 0) {
            ...
            document.getElementById('gold-nisab-value').textContent = 'يجب إدخال سعر الذهب';
            document.getElementById('trade-nisab-value').textContent = 'يجب إدخال سعر الذهب';
            document.getElementById('metals-nisab-value').textContent = 'يجب إدخال سعر الذهب';
        }
    }

    // استدعاء وظيفة تحديث النصاب عند تحميل الصفحة
    updateNisabValues();
    ...
    document.getElementById('update-gold-price').addEventListener('click', function() {
        ...
        if (!isNaN(newGoldPrice) && newGoldPrice > 0) {
            ...
            updateNisabValues();
            alert(`تم تحديث سعر الذهب إلى ${newGoldPrice} ريال للجرام`);
        } else {
            alert("يرجى إدخال سعر صحيح لجرام الذهب");
        }
    });
    ...
    document.getElementById('update-silver-price').addEventListener('click', function() {
        ...
        if (!isNaN(newSilverPrice) && newSilverPrice > 0) {
            ...
            alert(`تم تحديث سعر الفضة إلى ${newSilverPrice} ريال للجرام`);
        } else {
            alert("يرجى إدخال سعر صحيح لجرام الفضة");
        }
    });
    ...
    setTimeout(() => {
        showNotification('مرحبًا بكم في حاسبة الزكاة الإسلامية، نسأل الله العلي القدير أن يجعلها في ميزان حسناتكم', 'success', 8000);
    }, 1000);
});